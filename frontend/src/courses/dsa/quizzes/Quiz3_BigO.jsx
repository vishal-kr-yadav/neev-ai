...
const questions = [
  { q: 'What is the Big O of searching every element in an unsorted array?', options: ['O(1)', 'O(log n)', 'O(n)', 'O(n²)'], answer: 2 },
  { q: 'A nested loop (loop inside a loop) over n elements has time complexity:', options: ['O(n)', 'O(n²)', 'O(2n)', 'O(log n)'], answer: 1 },
  { q: 'Binary search has time complexity:', options: ['O(n)', 'O(n²)', 'O(1)', 'O(log n)'], answer: 3 },
  { q: 'If an algorithm takes 1 second for n=1000, and it\'s O(n²), approximately how long for n=10000?', options: ['10 seconds', '1000 seconds', '100 seconds', '50 seconds'], answer: 2 },
  { q: 'Which Big O is the fastest for large n?', options: ['O(n)', 'O(n²)', 'O(log n)', 'O(n log n)'], answer: 2 },
]

export default function Quiz3_BigO({ onComplete }) {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: 40 }}>
      ...
      <p style={{ color: 'var(--text-muted)' }}>{score >= 3 ? 'You\'ve got a solid grasp of Big O notation!' : 'Review the topic and try again!'}</p>
    </motion.div>
  )

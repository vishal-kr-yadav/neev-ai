...
const questions = [
  { q: 'In a Binary Tree, each node has at most:', options: ['1 child', '3 children', '4 children', '2 children'], answer: 3 },
  { q: 'In-Order traversal visits nodes in which order?', options: ['Root → Left → Right', 'Right → Root → Left', 'Left → Right → Root', 'Left → Root → Right'], answer: 3 },
  { q: 'A leaf node is:', options: ['The root of the tree', 'A node with exactly one child', 'A node with no children', 'A node with two children'], answer: 2 },
  { q: 'The height of a tree with a single node (root only) is:', options: ['1', '2', '-1', '0'], answer: 3 },
  { q: 'A Perfect Binary Tree with height h has how many nodes?', options: ['2^h', 'h² + 1', '2h + 1', '2^(h+1) - 1'], answer: 3 },
]

export default function Quiz14_BinaryTrees({ onComplete }) {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: 40 }}>
      ...
      <p style={{ color: 'var(--text-muted)' }}>{score >= 3 ? 'Tree traversal pro!' : 'Review the topic and try again!'}</p>
    </motion.div>
  )

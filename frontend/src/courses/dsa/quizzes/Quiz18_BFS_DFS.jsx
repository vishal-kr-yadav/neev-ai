...
const questions = [
  { q: 'BFS uses which data structure?', options: ['Stack', 'Queue', 'Priority Queue', 'Linked List'], answer: 1 },
  { q: 'DFS uses which data structure?', options: ['Queue', 'Heap', 'Stack (or recursion)', 'Hash Table'], answer: 2 },
  { q: 'Which algorithm guarantees the shortest path in an unweighted graph?', options: ['DFS', 'BFS', 'Topological Sort', 'Prim\'s algorithm'], answer: 1 },
  { q: 'BFS explores a graph:', options: ['By always following the deepest unvisited node first', 'Randomly selecting nodes to visit', 'Level by level (all neighbors before going deeper)', 'By following edges in reverse order'], answer: 2 },
  { q: 'The time complexity of both BFS and DFS is:', options: ['O(V²)', 'O(V log V)', 'O(E log V)', 'O(V + E)'], answer: 3 },
]

export default function Quiz18_BFS_DFS({ onComplete }) {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: 40 }}>
      ...
      <p style={{ color: 'var(--text-muted)' }}>{score >= 3 ? 'Graph traversal expert!' : 'Review the topic and try again!'}</p>
    </motion.div>
  )

...
const questions = [
  { q: '95% normal + 5% failure records is called:', options: ['Data noise', 'Class imbalance', 'Overfitting', 'Missing data'], answer: 1 },
  { q: 'Correlation 0.85 between vibration and temperature means:', options: ['One causes the other', 'They tend to increase together', 'They are identical', 'No relationship'], answer: 1 },
  { q: 'Bimodal temperature distribution likely indicates:', options: ['Sensor error', 'Two distinct operating modes', 'Equipment failure', 'Random noise'], answer: 1 },
  { q: 'Why is EDA important before modeling?', options: ['It trains the model', 'It reveals data quality issues and patterns', 'It deploys the system', 'It replaces cleaning'], answer: 1 },
  { q: 'A sensor outlier should be:', options: ['Always removed', 'Always kept', 'Investigated — could be error OR real signal', 'Ignored'], answer: 2 },
]

export default function Quiz3_EDA({ onComplete }) {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: 40 }}>
      ...
      <p style={{ color: 'var(--text-muted)' }}>{score >= 3 ? 'Data explorer!' : 'Review the topic and try again!'}</p>
    </motion.div>
  )

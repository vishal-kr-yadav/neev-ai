...
const questions = [
  { q: 'Model drift occurs when:', options: ['Server restarts', 'Feature-failure relationship changes over time', 'Dashboard crashes', 'Sensors are replaced'], answer: 1 },
  { q: 'Champion-Challenger approach:', options: ['Two teams compete', 'Compare current vs new model before swapping', 'Train two algorithms', 'Delete old model'], answer: 1 },
  { q: 'Most common ML serving method:', options: ['Email predictions', 'REST API endpoint', 'Printed reports', 'Fax'], answer: 1 },
  { q: 'PdM model retrain frequency:', options: ['Every hour', 'When drift detection shows degradation', 'Never', 'Every minute'], answer: 1 },
  { q: 'NOT part of production PdM:', options: ['FastAPI service', 'React dashboard', 'Jupyter notebook on laptop', 'Model registry'], answer: 2 },
]

export default function Quiz13_Deployment({ onComplete }) {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: 40 }}>
      ...
      <p style={{ color: 'var(--text-muted)' }}>{score >= 3 ? 'Deployment ready!' : 'Review the topic and try again!'}</p>
    </motion.div>
  )

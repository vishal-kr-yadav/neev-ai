...
const questions = [
  { q: 'Random Forest predicts by:', options: ['One deep tree', 'Many trees voting together', 'Neural network', 'Linear regression'], answer: 1 },
  { q: 'XGBoost vs Random Forest:', options: ['Same algorithm', 'XGBoost builds trees sequentially correcting errors', 'XGBoost is simpler', 'Random Forest is newer'], answer: 1 },
  { q: 'LightGBM is faster because:', options: ['Uses less data', 'Grows trees leaf-wise not level-wise', 'Has fewer features', 'Skips training'], answer: 1 },
  { q: 'For small dataset (<10K):', options: ['XGBoost only', 'Random Forest — robust and less overfitting', 'Deep learning', 'No model works'], answer: 1 },
  { q: 'Feature importance tells you:', options: ['Which data to delete', 'Which features contribute most to prediction', 'Model speed', 'Sensor cost'], answer: 1 },
]

export default function Quiz10_Models({ onComplete }) {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: 40 }}>
      ...
      <p style={{ color: 'var(--text-muted)' }}>{score >= 3 ? 'Model arena champion!' : 'Review the topic and try again!'}</p>
    </motion.div>
  )

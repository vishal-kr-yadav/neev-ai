...
const questions = [
  { q: '75% failure probability maps to:', options: ['LOW', 'MEDIUM', 'HIGH — schedule within 7 days', 'CRITICAL'], answer: 2 },
  { q: 'Why risk tiers over raw probability?', options: ['Tiers are more accurate', 'Workers need actionable categories not numbers', 'Tiers use less memory', 'Probabilities are wrong'], answer: 1 },
  { q: 'Cost of False Negative vs False Positive:', options: ['Same cost', 'FN is 10-20x more costly — missed failures cause downtime', 'FP is worse', 'Neither matters'], answer: 1 },
  { q: 'Cost-based threshold minimizes:', options: ['Model complexity', 'Total expected cost of both false alarms and missed failures', 'Training time', 'Feature count'], answer: 1 },
  { q: 'Priority matrix plots probability against:', options: ['Time', 'Business impact cost if it fails', 'Sensor count', 'Equipment age'], answer: 1 },
]

export default function Quiz12_Risk({ onComplete }) {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: 40 }}>
      ...
      <p style={{ color: 'var(--text-muted)' }}>{score >= 3 ? 'Risk scoring pro!' : 'Review the topic and try again!'}</p>
    </motion.div>
  )

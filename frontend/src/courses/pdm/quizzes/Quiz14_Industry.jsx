...
const questions = [
  { q: 'Most sensor data per unit:', options: ['HVAC', 'Wind turbine', 'Aviation — 5TB+ per flight', 'Solar panel'], answer: 2 },
  { q: 'Most expensive wind turbine component:', options: ['Blade', 'Gearbox — requires crane + ₹50L', 'Generator', 'Tower'], answer: 1 },
  { q: 'Healthcare PdM uniquely critical because:', options: ['Equipment is expensive', 'Failure directly impacts patient safety', 'Hospitals are large', 'Doctors are busy'], answer: 1 },
  { q: 'Universal PdM pipeline:', options: ['Buy sensors → wait for failure', 'Collect → Clean → Features → Train → Predict → Act', 'Just use AI', 'Replace all equipment'], answer: 1 },
  { q: 'Highest PdM ROI industry:', options: ['Retail', 'Oil and gas — ₹1-5Cr/day downtime cost', 'Education', 'Agriculture'], answer: 1 },
]

export default function Quiz14_Industry({ onComplete }) {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: 40 }}>
      ...
      <p style={{ color: 'var(--text-muted)' }}>{score >= 3 ? 'Industry expert!' : 'Review the topic and try again!'}</p>
    </motion.div>
  )

...
const questions = [
  { q: 'Which maintenance type uses sensor data + ML to predict failures?', options: ['Reactive', 'Preventive', 'Predictive', 'Prescriptive'], answer: 2 },
  { q: 'Main disadvantage of reactive maintenance?', options: ['High sensor costs', 'Unplanned downtime causes max production loss', 'Too many scheduled stops', 'Requires ML expertise'], answer: 1 },
  { q: 'PdM typically reduces maintenance costs by:', options: ['5-10%', '25-40%', '70-80%', '95-100%'], answer: 1 },
  { q: 'NOT a PdM data source:', options: ['Vibration sensors', 'Work order history', 'Employee satisfaction surveys', 'Asset age profiles'], answer: 2 },
  { q: 'PdM ROI comes primarily from:', options: ['Selling sensor data', 'Reducing unplanned downtime', 'Eliminating all maintenance', 'Replacing all equipment'], answer: 1 },
]

export default function Quiz1_PdM({ onComplete }) {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: 40 }}>
      ...
      <p style={{ color: 'var(--text-muted)' }}>{score >= 3 ? 'PdM foundations solid!' : 'Review the topic and try again!'}</p>
    </motion.div>
  )

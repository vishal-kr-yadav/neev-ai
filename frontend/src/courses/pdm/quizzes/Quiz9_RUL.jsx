...
const questions = [
  { q: 'RUL prediction is:', options: ['Classification', 'Regression — predicting continuous days-to-failure', 'Clustering', 'Unsupervised'], answer: 1 },
  { q: '87% probability + 7 days RUL vs 72% + 45 days — fix first:', options: ['Second pump', 'First pump — higher probability AND imminent', 'Neither', 'Both simultaneously'], answer: 1 },
  { q: 'MAE of 5 days means:', options: ['Model is 5% accurate', 'Prediction is off by 5 days on average', '5 failures predicted', 'Model trained for 5 days'], answer: 1 },
  { q: 'Confidence interval [15, 31] days means:', options: ['Exactly 23 days', '80% confident failure between 15-31 days', 'Already failed', 'Will never fail'], answer: 1 },
  { q: 'Classification + RUL together provide:', options: ['Redundant info', 'Complete picture — IF and WHEN for scheduling', 'Slower predictions', 'Worse accuracy'], answer: 1 },
]

export default function Quiz9_RUL({ onComplete }) {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: 40 }}>
      ...
      <p style={{ color: 'var(--text-muted)' }}>{score >= 3 ? 'RUL prediction mastered!' : 'Review the topic and try again!'}</p>
    </motion.div>
  )

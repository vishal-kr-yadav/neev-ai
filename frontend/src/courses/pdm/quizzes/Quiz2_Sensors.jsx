...
const questions = [
  { q: 'Best sensor for bearing wear detection?', options: ['Temperature', 'Pressure', 'Vibration accelerometer', 'Flow meter'], answer: 2 },
  { q: 'Sudden increase in motor current draw indicates:', options: ['Sensor malfunction', 'Increased mechanical load or degradation', 'Normal operation', 'Power grid issue'], answer: 1 },
  { q: 'Industrial sensor sampling rate:', options: ['Once per day', '1-100 readings per second', 'Once per hour', '1 million per second'], answer: 1 },
  { q: 'A sensor "baseline" is:', options: ['Maximum allowed reading', 'Normal operating range for healthy equipment', 'Minimum possible value', 'Factory setting'], answer: 1 },
  { q: 'Refrigerant leaks detected by:', options: ['Vibration sensor', 'Acoustic sensor', 'Pressure transducer', 'Camera'], answer: 2 },
]

export default function Quiz2_Sensors({ onComplete }) {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: 40 }}>
      ...
      <p style={{ color: 'var(--text-muted)' }}>{score >= 3 ? 'Sensor expert!' : 'Review the topic and try again!'}</p>
    </motion.div>
  )

...
const questions = [
  { q: 'Baseline deviation measures:', options: ['Absolute sensor value', 'How far current reading is from normal as percentage', 'Sensor age', 'Repair cost'], answer: 1 },
  { q: 'Decreasing MTBF means:', options: ['Equipment is improving', 'Failures are becoming more frequent', 'Sensors are miscalibrated', 'Maintenance is too frequent'], answer: 1 },
  { q: 'Lifecycle ratio 0.95 means:', options: ['5% of life used', '95% of expected life used', 'Equipment is new', '95% failure probability'], answer: 1 },
  { q: 'Repair acceleration captures:', options: ['Speed of repairs', 'Whether maintenance is becoming more urgent over time', 'Motor speed', 'Data processing rate'], answer: 1 },
  { q: 'Rolling statistics beat raw values because:', options: ['They use less memory', 'They smooth noise and capture trends', 'They are faster to compute', 'They eliminate outliers'], answer: 1 },
]

export default function Quiz7_Features({ onComplete }) {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: 40 }}>
      ...
      <p style={{ color: 'var(--text-muted)' }}>{score >= 3 ? 'Feature engineering master!' : 'Review the topic and try again!'}</p>
    </motion.div>
  )

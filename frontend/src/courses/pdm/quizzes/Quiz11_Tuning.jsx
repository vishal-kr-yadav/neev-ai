...
const questions = [
  { q: 'Parameters vs hyperparameters:', options: ['Same thing', 'Parameters learned during training hyperparameters set before', 'Parameters are faster', 'Hyperparameters are optional'], answer: 1 },
  { q: 'Grid search 4 values × 3 hyperparameters:', options: ['12 combinations', '64 combinations', '7 combinations', '4 combinations'], answer: 1 },
  { q: 'Bayesian optimization is faster because:', options: ['Uses less data', 'Learns from previous trials to focus on promising areas', 'Skips validation', 'Uses random values'], answer: 1 },
  { q: '5-fold cross-validation:', options: ['5 separate models', 'Data split 5 ways each takes turn as test set', '5% test data', 'Trains 5 times faster'], answer: 1 },
  { q: 'Better on train than test data means:', options: ['Great model', 'Overfitting — memorized training data', 'Underfitting', 'Perfect generalization'], answer: 1 },
]

export default function Quiz11_Tuning({ onComplete }) {
  ...
  if (done) return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ textAlign: 'center', padding: 40 }}>
      ...
      <p style={{ color: 'var(--text-muted)' }}>{score >= 3 ? 'Tuning expert!' : 'Review the topic and try again!'}</p>
    </motion.div>
  )
